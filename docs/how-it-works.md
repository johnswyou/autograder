...
Agentic Autograder does not pass a PDF filename to a model and hope it finds the
right answer. Python opens the documents, renders PDF pages as JPEG images,
and sends those images through OpenRouter Chat Completions. The model first learns
...
Python supplies sharper crops and the model transcribes and grades the work.
...
There is no standalone OCR engine. The same selected-model visual path does
...
then base64-encodes those JPEG bytes in `image_url` content parts whose URLs
begin `data:image/jpeg;base64,`.
...
If a PDF has embedded text, the model can ask Python for that text layer. This is
useful for exact typeset prompts; it is only an additional reading tool and is
not the primary path for handwriting.

### 2. The model maps from full pages before anyone transcribes

The model receives the known problem inventory from the blank assignment and
...
cropped and may be enlarged within an upscale limit. The model can also request a
...
### 4. The model transcribes the located work

A fresh model task receives the Problem 1(c) prompt and its mapped crop or
...
`autograder/ocr.py` orchestrates the model through OpenRouter Chat Completions; it
does not invoke Tesseract, PyMuPDF OCR, or another recognition engine.

### 5. The model applies the rubric

A separate model grading task receives the fixed rubric criteria, official
...
The model decides how the evidence satisfies each rubric criterion, writes a
...
The model returns typed structured data rather than prose that Python must guess
how to parse. Pydantic schemas reject malformed results and give it a chance
...
that the model read the handwriting correctly or made the right academic judgment.
...
|---|---|
| Interpret assignment and submission page images | Selected model |
| Map each problem to work across full pages | Selected model |
| Transcribe handwriting and report confidence | Selected model |
| Judge work against rubric criteria and write feedback | Selected model |
